...
4. **검토한다.** 아래 "강의 노트 검토 기준"으로 노트 전체를 본다. `npm run build`로
   `check-lecture-notes.mjs` 통과를 확인한다. 사용자가 하나씩 검토하는 단계에서는 지적한
   부분을 중심으로 수정하고, 관련 용어·링크·선행 설명에 필요한 수정까지 연결한다. 다른 노트
   전면 개편은 새로 시작하지 않는다.
...
작성자의 자체 검토, 도구 간 리뷰(`agent-workflow.md`의 자동 리뷰 루프), 사용자 검토가 모두
이 기준을 쓴다. 출발점은 노트의 의도다. **수업에 오지 않은 학생이 노트만 읽고도 그 주차
내용을 높은 수준으로 이해하고, 자기 프로젝트에 적용할 수 있어야 한다.** 슬라이드처럼
강의자의 말을 전제한 요약이 아니라, 글만으로 설명이 완결되는 읽을거리다. 노트 하나뿐
아니라 앞뒤 주차와의 연결까지 본다.

1. **사실.** 수치·인용·쪽 번호·서지정보가 원자료와 맞는가. 원자료로 뒷받침되지 않는 주장,
   근거보다 강한 해석은 없는가. 원자료의 보고값, 그 값으로 새로 계산한 것, 교육용 가상
   예시가 구분되는가. 원자료 자체의 오류는 조용히 따르지 않고 밝힌다. 문헌이 엇갈리거나
   아직 정설이 없는 대목을 한쪽으로 단정하지 않고 그렇다고 말하는가.
2. **출처.** 각주마다 실제로 열어 저자·연도·제목·venue·arXiv 번호를 대조한다. 인용된 자리의
   주장이 그 자료의 그 위치(절·표·그림 번호)에 실제로 있는가. 비슷한 주제의 다른 논문이나
   2차 해설이 원논문 자리에 들어가 있지 않은가. 링크가 열리고 안정적 식별자를 가리키는가.
   과목별 참고문헌 규칙(예: 1번 = 대표 서베이)을 지키는가. 그림·영상의 라이선스와 출처 표기가
   맞는가.
3. **흐름.** 그 주차의 목표가 하나의 줄기로 이어지는가. 각 절이 왜 거기 있는지, 앞 절에서
   무엇을 받아 다음 절로 넘기는지 드러나는가. 선행 개념이 쓰이기 전에 설명되는가. 같은
   논점이 반복되지 않는가. 지금 순서와 구성이 최선인지, 더 나은 배치가 있는지 따진다.
   **제목 계층**도 흐름의 일부다. 목차만 읽어도 노트의 줄기가 보여야 한다. 상위 제목은 하위
   제목들을 포괄하고, 하위 제목은 상위 제목을 되풀이하지 않는다. 같은 수준의 제목은 한 축(예:
   분석 단계)과 한 형식(개념 중심 명사구)으로 맞추고, 콜론 부제·질문형·동작형("~하기",
   "~읽는 법")을 섞지 않는다.
4. **노트 사이의 연결.** 같은 주차와 앞뒤 주차의 노트가 용어·기호·모델 버전·수치를 같게
   쓰는가. 한 개념이 두 노트에 걸칠 때 빠진 조각이나 겹치는 조각이 없는가. 노트 간 링크가
   맞는 노트의 맞는 절을 가리키는가. 주차 도입의 지도(표·계보 요약)가 실제 노트 구성과
   일치하는가. 앞 주차에서 설명한 것은 다시 풀어 쓰지 않고 짧은 복습과 링크로 받는가.
5. **읽기만으로의 이해.** 추상적인 설명마다 구체적인 예가 따라오는가. 어려운 계산이나
   메커니즘을 실제 값으로 한 단계씩 밟아 보이는가(worked example). 용어·변수·약어를 처음
   쓸 때 정의하는가. 독자가 "왜?"라고 묻게 되는 비약은 없는가. 토론 질문이 본문에서 답을
   찾을 재료를 주는가. 어려운 대목일수록 산문만으로 넘기지 않고 예시·도식이 붙어 있는가.
6. **시각 자료.** 글보다 그림·표·영상이 빨리 전달하는 곳에 시각 자료가 있는가. 기존의 좋은
   자료(원논문 그림, 공식 교재, 강연 영상)를 먼저 찾았고 라이선스를 확인했는가. 자작 도식은
   대응 자료가 없을 때만 쓰며, 실제 데이터나 사례 값으로 그렸는가. 그림이 본문의 설명과
   같은 기호·같은 예시를 쓰고, 캡션만 읽어도 무엇을 보라는 것인지 알 수 있는가.
7. **수업 적합성.** 핵심과 선택 심화(`<Details>`)가 구분되는가. 수업 중 활동은 범위와 완료 기준이
   분명한가. 분량을 수업 시간에 맞추지 않는다. 노트가 읽을거리로 완결되는 것이 우선이며, 분량
   조정이나 더 구체적인 서술은 사용자가 요청할 때만 한다.
8. **문장.** 사람이 처음부터 우리말로 쓴 글처럼 읽히는가. 소리 내 읽어 걸리는 곳은 없는가.
   번역투("~에 있어서", "~되어진다", "~라고 할 수 있다"의 남발), 같은 어미·접속사·문형의
   연속, 명사만 이어 붙인 문장, 영어 용어에 어색하게 붙은 조사, 구호식 단문과
   `**굵게**`·불릿 남발, 요지가 둘 이상 섞인 문단을 본다. 문단의 첫 문장이 그 문단의
   요지를 말하는가. 기계적으로 균일한 문장 길이와 매 절 같은 마무리 문장도 손본다.
9. **작성 규칙.** 이 문서의 표기·인용·저작권·컴포넌트 규칙을 지키는가. 수식 기호와 용어가
   일관되는가, 각주 `[^키]`와 정의가 맞고 인용이 그 자리의 주장을 뒷받침하는가, 약어 full form,
   과한 단정과 기법·단계 서열 매기기, em dash, 문단 이식이나 그림 라이선스 누락, heading의
   수식, MDX 렌더 오류가 없는가. 렌더는 개발 서버에서 실제 페이지를 열어 확인한다(수식·SVG·
   각주 번호·목차 줄바꿈).

**반복 검토.** 한 라운드는 노트 하나를 처음부터 끝까지 읽고 위 기준을 모두 적용하는 것이다.
지적은 `[심각도] 파일:줄 — 문제 — 근거 — 제안`으로 남기고, 수정한 뒤에는 개발 서버에서 렌더를
확인한다. 다음 라운드는 고친 자리만 다시 보지 않고 노트 전체를 다시 읽는다(수정이 흐름을
바꾼다). 새 지적이 없는 라운드가 나오면 끝낸다. 라운드 기록은 repo 밖에 둔다.
