...
2. **자료조사를 꼼꼼히 한다.**
   ...
   - **재사용 경계.** 남의 글은 읽고 **우리 말로 다시 쓴다** — 문단·문장을 번역하거나
     발췌해 본문에 싣지 않는다(CC-BY-NC-ND 등 ND 라이선스·저작권 있는 교재는 특히).
     번역·각색·요약은 2차적저작물이라 ND 라이선스가 배포를 금지한다. 개념·방법·프레임을
     설명하는 건 자유(아이디어는 저작권 대상 아님), 그 **표현**을 옮기는 게 문제다.
     꼭 필요한 짧은 구절만 따옴표로 묶어 출처와 함께 인용한다(저작권법 §28). 그림·표는
     **원본 그대로**(자르기·재작도·라벨 변경 없이) `<Figure>` 로 참조하고, `source` 에
     저작자·출처·라이선스·**"수정 없음"** 을 적는다. 라이선스가 버전을 안 밝히면 우리도
     버전을 단정하지 않는다.
3. **작성한다.**
...
4. **검토한다.** 수식 기호 일관성, 각주 참조(`[^키]`) ↔ 정의(`[^키]:`) 매칭·인용↔주장 적합성, 용어 통일,
...
- 정의 본문은 **마크다운** — `[제목](url)` 링크·`*이탤릭*`·`$수식$` 다 된다.
  **한 노트 안에서 형식을 통일**한다: `저자(성, 이니셜). (연도). [제목](링크).
  *매체/venue*. — 이 인용이 뒷받침하는 내용·절·표·그림.` 저자명은 로마자, 제목은
  원어 그대로 둔다(한국어 노트여도 번역하지 않는다).
