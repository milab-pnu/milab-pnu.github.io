...
2. **자료조사를 꼼꼼히 한다.**
   - **1차 출처(원논문)** 로 사실·수식·수치를 확정한다. 개념은 그것을 처음
     ...
     서지정보는 저자·연도·학회/저널·페이지·arXiv 번호까지 정확하게 쓴다. 특정 절·그림을
     읽어야 이해되는 경우에만 본문이나 그림 캡션에서 안내한다. 각주 항목에 해설을 덧붙이지 않는다.
   ...
   - **무료 접근본을 우선**한다(arXiv·open-access·저자 PDF·공식 무료판). 페이월이면
     무료 대안을 병기한다. 블로그·공식 문서에는 확인 가능한 발행·수정 연도나 버전을 적되,
     접근일·확인일은 기재하지 않는다. 방문한 연도를 발행 연도로 대신 쓰지 않는다.
...
3. **작성한다.**
   - **기존 주제와 노트 제목은 유지한다.** 가독성을 고친다는 이유로 주제나 수업 범위를
     바꾸지 않는다.
   - **제목은 개념 중심의 짧은 명사구로 쓴다.** 본문은 `##`, 하위 절은
     `###`로 논리적 계층을 표현하고, 번호 형식은 해당 과목의 규칙을 따른다. 모든 내용을 같은 수준의
     제목으로 나열하거나 문장형 제목을 반복하지 않는다. 좌측 목차의 줄바꿈도 확인한다.
   - **선행 개념을 사용하는 곳보다 먼저 설명한다.** 뒤 주차에 예정된 내용이어도 현재
     설명에 필요하면 최소한의 기초를 먼저 둔다.
     뒤 주차는 짧은 복습과 링크에서 출발해 심화한다.
   ...
   - **작성하는 설명에서 em dash로 문장을 잇거나 부연하지 않는다.** 본문·제목·캡션·
     출처 모두 문장을 나누거나 쉼표·괄호·콜론 등 문맥에 맞는 표현을 쓴다. 출처의 정식 제목과
     직접 인용문은 원문 표기를 보존하고, 페이지 범위의 en dash나 기술 용어의 하이픈과 구분한다.
   ...
   - **문제 상황·작동 원리·작은 예시를 연결한다.** 모든 절에 같은 제목과 설명 틀을
     기계적으로 반복하지 않는다. 예시는 무엇이 문제인지, 적용 전후 무엇이 달라지는지
     보이게 하고, 교육용 가정·수치와 실제 모델의 결과를 구분한다.
   - 용어는 기본적으로 **영어**(technical term). 한국어로 쓰는 핵심 용어는 각 노트의
     첫 설명에 영어를 괄호로 병기한다(예: 시퀀스 모델(sequence model), 말뭉치(corpus),
     외삽(extrapolation), 처리량(throughput)). 어색하거나 모호한 번역은 자연스러운
     표현으로 고치고 원어를 함께 둔다. 이미 설명한 용어마다 괄호를 반복하거나 일반
     문장까지 영어로 번역하지 않는다. 원어의 의미도 구분한다(sequence model과
     sequential computation은 다르다).
...
4. **검토한다.** 수식 기호 일관성, 각주 `[^키]` ↔ 정의 매칭·인용↔주장 적합성, 용어 통일,
   ...
   `check-lecture-notes.mjs` 통과. 노트 하나뿐 아니라 **해당 주차 전체의 연결성·시각 자료의
   충분성·가독성·중복과 누락**을 함께 점검한다. 지정된 수업 시간 안에서 설명 가능한지
   보고, 핵심 설명과 선택 심화 자료를 구분한다. 내용의 위치는 학습 흐름으로 판단한다.
   사용자가 하나씩 검토하는 단계에서는 지적한 부분을 중심으로 수정하고, 관련 용어·링크·
   선행 설명에 필요한 수정까지 연결한다. 다른 노트 전면 개편은 새로 시작하지 않는다.
...
- **출처는 서지정보와 링크 중심으로 간결하게 쓴다.** 참고문헌 뒤에 내용 요약, 인용 이유,
  절·표·그림 목록, 수업 활용 설명을 붙이지 않는다. 기존 em dash 뒤 해설도 삭제한다.
  접근일·열람일·확인일은 참고문헌과 본문 모두에서 생략한다. 발행 연도·판본 등 출처
  식별 정보와 이미지의 라이선스·수정 여부는 보존한다.
...
- 정의 본문은 **마크다운**(`[제목](url)`·`*이탤릭*`·`$수식$`). **한 노트 안에서 형식
  통일**: `저자(성, 이니셜). (연도). 제목. *매체/venue*. 권(호), 페이지. [링크](URL).`
...
- **CC 자료도 개별 이용 조건을 확인한다.** 무료 공개·교육 목적만으로 NC 충족을 단정하지
  않는다. 저자·출처·저작권 표시와 라이선스 링크를 확인하고, 별도 권리 표시가 있는 그림도
  살핀다. ND는 번역·변형본의 공유를 허용하지 않는다. 변경 없는 발췌의 이용도 맥락과 다른
  조건을 확인한다([CC 공식 FAQ](https://creativecommons.org/faq/)). 버전은 추정하지 않으며,
  이용 근거를 확정하기 어려우면 원문 페이지 링크로 안내한다.
- **출판 논문·arXiv 그림도 개별 권리와 이용 조건을 확인한다.** 직접 분석하는 인용이라도
  분량·목적·주종 관계 등 적용 요건을 검토한다. 다시 그렸다는 이유만으로 이용 문제가
  해결되지는 않는다. 개념을 독립적으로 설명하거나 원문을 링크한다.
...
- 자르기·라벨 번역·재그리기도 변형에 해당할 수 있다. 자작 도식은 원본의 표현·배치를
  옮기는 대신 개념을 독립적으로 구성한다. **핫링크도 저작권 판단 대상**(면책 아님).
...
1. 교재 해당 장을 각 주차 노트 안에서 **지정 읽기**로 링크한다.
...
이 문서가 과목 공통 작성 규칙의 정본이다. 과목별 합의는 해당 과목 repo의 `AGENTS.md`에
기록하고 커밋한다. 공통 규칙·함정을 알게 되면 **해당 절에
