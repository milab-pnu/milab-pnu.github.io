...
3. **작성한다.**
   - **기존 주제와 노트 제목은 유지한다.** 가독성을 고친다는 이유로 주제나 수업 범위를
     바꾸지 않는다. 별도 부제목은 기본으로 붙이지 않는다.
   - **제목은 개념 중심의 짧은 명사구로 쓴다.** 본문은 `## 1. …`, 하위 절은
     `### 1-1. …`, `### 1-2. …`처럼 논리적 계층으로 묶는다. 모든 내용을 같은 수준의
     제목으로 나열하거나 문장형 제목을 반복하지 않는다. 좌측 목차의 줄바꿈도 확인한다.
   - **선행 개념을 사용하는 곳보다 먼저 설명한다.** 뒤 주차에 예정된 내용이어도 현재
     설명에 필요하면 최소한의 기초를 먼저 둔다(예: 학습 메모리 계산 전 FP32·FP16·BF16).
   ...
   - **확인 문제·퀴즈와 별도 수업 진행안은 요청받지 않으면 만들지 않는다.** 개념을
     설명하는 풀이 예제와 실제 과제·프로젝트·평가 안내는 이에 해당하지 않는다.
...
4. **검토한다.** 수식 기호 일관성, 각주 `[^키]` ↔ 정의 매칭·인용↔주장 적합성, 용어 통일,
   ...
   (예: 데이터 구성의 의미와 실행 효율의 설명을 나누되, 중복 대신 서로 연결).
...
│   ├── 01a-topic.md       # intro 다음 첫 기술 노트 (order 로 정렬)
│   ├── 02a-topic.md       # 2주차부터 a로 시작, b·c 순서로 추가
│   ├── 06a-topic.mdx     # 노트가 하나뿐인 주차도 a로 시작
│   └── assets/            # 노트에 넣는 이미지 (상대경로 참조)
├── .github/workflows/notify.yml   # 손대지 않음
└── .gitattributes                 # 손대지 않음
```

**파일명은 주차 두 자리 + 소문자 순번 + 주제**로 정한다(`02a-…`, `02b-…`, `06a-…`).
1주차 수업 소개는 `01-intro`처럼 순번 없는 이름을 쓸 수 있다. 기존 소개 파일 이름은
별도 요청 없이 바꾸지 않는다. 이름을 바꿀 때는 `week`·`order`의 정렬 관계를 유지하고,
...
- **각 웹 노트의 참고문헌 1번은 해당 주제의 대표 서베이 논문으로 둔다.** 도입에서
  서베이가 제공하는 전체 맥락과 읽을 범위를 안내하며 가장 먼저 인용한다. 각주 정의를
  맨 위로 옮기는 것만으로는 번호가 바뀌지 않는다. 개별 수식·구조·실험 결과는 이를
  직접 뒷받침하는 원논문을 추가로 인용하고, 빌드 결과에서도 서베이가 1번인지 확인한다.
