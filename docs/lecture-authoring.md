...
1. **사실.** 수치·인용·쪽 번호·서지정보가 원자료와 맞는가. 원자료로 뒷받침되지 않는 주장,
   ...
   예시가 구분되는가. 원자료 자체의 오류는 조용히 따르지 않고 밝힌다.
2. **흐름.** 그 주차의 목표가 하나의 줄기로 이어지는가. 각 절이 왜 거기 있는지, 앞 절에서
   ...
   **제목 계층**도 흐름의 일부다. 목차만 읽어도 노트의 줄기가 보여야 한다. 상위 제목은 하위
   ...
   "~읽는 법")을 섞지 않는다.
3. **읽기만으로의 이해.** 추상적인 설명마다 구체적인 예가 따라오는가. 어려운 계산이나
   메커니즘을 실제 값으로 한 단계씩 밟아 보이는가(worked example). 용어·변수·약어를 처음
   쓸 때 정의하는가. 독자가 "왜?"라고 묻게 되는 비약은 없는가. 토론 질문이 본문에서 답을
   찾을 재료를 주는가.
4. **시각 자료.** 글보다 그림·표·영상이 빨리 전달하는 곳에 시각 자료가 있는가. 기존의 좋은
   자료(원논문 그림, 공식 교재, 강연 영상)를 먼저 찾았고 라이선스를 확인했는가. 자작 도식은
   대응 자료가 없을 때만 쓰며, 실제 데이터나 사례 값으로 그렸는가.
5. **수업 적합성.** 지정된 수업 시간 안에 다룰 수 있는 분량인가. 핵심과 선택 심화(`<Details>`)가
   구분되는가. 수업 중 활동은 범위와 완료 기준이 분명한가.
6. **작성 규칙.** 이 문서의 표기·인용·저작권·컴포넌트 규칙을 지키는가. 수식 기호와 용어가
   일관되는가, 각주 `[^키]`와 정의가 맞고 인용이 그 자리의 주장을 뒷받침하는가, 약어 full form,
   과한 단정과 기법·단계 서열 매기기, em dash, 소리 내 읽어 걸리는 번역투, 문단 이식이나 그림
   라이선스 누락, heading의 수식, MDX 렌더 오류가 없는가.
