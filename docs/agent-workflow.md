...
1. 작성자가 커밋한다. **리뷰어에게 `HANDOFF.md`나 작성자의 요약·검토 포인트를 주지 않는다.**
...
2. 리뷰 도구를 **읽기 전용**으로 부른다(예: `codex exec -s read-only -C <과목 repo> -o <결과 파일>`).
   리뷰어는 문서 **전체**를 다음 기준으로 검토한다.
   - 사실이 맞는가, 틀리거나 원자료로 뒷받침되지 않는 정보는 없는가
   - 연결·구성·흐름이 지금이 최선인가
   - 강의 없이 읽기만 해도 높은 수준으로 이해되는가
   - 적절한 이미지·영상이 임베드되어 있는가(라이선스 포함)
...
- `HANDOFF.md`는 사람이 도구를 바꿔 이어서 작업할 때(아래 "인수인계")만 쓴다.
